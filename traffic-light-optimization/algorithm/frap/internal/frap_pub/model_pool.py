...
class ModelPool():
    def __init__(self, dic_path, dic_exp_conf):
        ...
        if os.path.exists(os.path.join(ROOT_DIR, self.dic_path["PATH_TO_WORK_DIRECTORY"], "best_model.pkl")):
            self.best_model_pool = pickle.load(
                open(os.path.join(ROOT_DIR, self.dic_path["PATH_TO_WORK_DIRECTORY"], "best_model.pkl"), "rb"))
        else:
    ...
    def single_test(self, cnt_round, external_configurations={}):
        ...
        dic_agent_conf = json.load(open(os.path.join(ROOT_DIR, records_dir, "agent.conf"), "r"))
        dic_exp_conf = json.load(open(os.path.join(ROOT_DIR, records_dir, "exp.conf"), "r"))
        dic_traffic_env_conf = json.load(open(os.path.join(ROOT_DIR, records_dir, "traffic_env.conf"), "r"))
        ...
        # dump dic_exp_conf
        if os.path.exists(os.path.join(ROOT_DIR, records_dir, "test_exp.conf")):
            json.dump(dic_exp_conf, open(os.path.join(ROOT_DIR, records_dir, "test_exp.conf"), "w"))
        ...
        if 1:
            ...
            if nan_num < nan_thres:
                ...
                f = open(os.path.join(ROOT_DIR, self.dic_path["PATH_TO_WORK_DIRECTORY"], "best_model_pool.log"), "a")
    ...
    def model_compare(self, cnt_round):
        ...
        dic_agent_conf = json.load(open(os.path.join(ROOT_DIR, records_dir, "agent.conf"), "r"))
        dic_exp_conf = json.load(open(os.path.join(ROOT_DIR, records_dir, "exp.conf"), "r"))
        dic_sumo_env_conf = json.load(open(os.path.join(ROOT_DIR, records_dir, "traffic_env.conf"), "r"))
        ...
        if 1:
            ...
            traffic_vol = get_traffic_volume(dic_exp_conf["TRAFFIC_FILE"][0], run_cnt)
            total_vol = get_total_traffic_volume(dic_exp_conf["TRAFFIC_FILE"][0])

            print(nan_num, nan_thres, self.best_model_pool)
            print(">>> ", vehicle_out, total_vol * 0.9)
            if vehicle_out > total_vol * 0.9:
                cnt = 0
                for i in range(len(self.best_model_pool)):
                    if self.best_model_pool[i][1] * 1.1 > dur:
                        break
                    cnt += 1
    ...
    def dump_model_pool(self):
        if self.best_model_pool:
            pickle.dump(self.best_model_pool,
                        open(os.path.join(ROOT_DIR, self.dic_path["PATH_TO_WORK_DIRECTORY"], "best_model.pkl"), "wb"))
