...
class NetworkAgent(Agent):
    ...
    def load_network(self, file_name, file_path=None):
        ...
        self.q_network = load_model(os.path.join(ROOT_DIR, file_path, "%s.h5" % file_name),
    ...
    def load_network_bar(self, file_name, file_path=None):
        if file_path == None:
            file_path = self.dic_path["PATH_TO_MODEL"]
        self.q_network_bar = load_model(os.path.join(ROOT_DIR, file_path, "%s.h5" % file_name),
                                        custom_objects={"Selector": Selector})
        print("succeed in loading model %s"%file_name)

    def save_network(self, file_name):
        self.q_network.save(os.path.join(ROOT_DIR, self.dic_path["PATH_TO_MODEL"], "%s.h5" % file_name))

    def save_network_bar(self, file_name):
        self.q_network_bar.save(os.path.join(ROOT_DIR, self.dic_path["PATH_TO_MODEL"], "%s.h5" % file_name))
