...
from algorithm.frap_pub.config import DIC_AGENTS
...
class Updater:
    ...
    def load_sample(self):
        ...
        if self.dic_exp_conf["PRETRAIN"]:
            sample_file = open(os.path.join(ROOT_DIR, self.dic_path["PATH_TO_PRETRAIN_WORK_DIRECTORY"],
                                            "train_round", "total_samples" + ".pkl"), "rb")
        elif self.dic_exp_conf["AGGREGATE"]:
            sample_file = open(os.path.join(ROOT_DIR, self.dic_path["PATH_TO_AGGREGATE_SAMPLES"],
                                            "aggregate_samples.pkl"), "rb")
        else:
            sample_file = open(os.path.join(ROOT_DIR, self.dic_path["PATH_TO_WORK_DIRECTORY"],
                                            "train_round", "total_samples" + ".pkl"), "rb")
        try:
            while True:
    ...
    def update_network(self):
        ...
        if self.dic_exp_conf["PRETRAIN"]:
            self.agent.q_network.save(os.path.join(ROOT_DIR, self.dic_path["PATH_TO_PRETRAIN_MODEL"],
                                    "%s.h5" % self.dic_exp_conf["TRAFFIC_FILE"][0]))
            shutil.copy(os.path.join(ROOT_DIR,
                self.dic_path["PATH_TO_PRETRAIN_MODEL"],"%s.h5" % self.dic_exp_conf["TRAFFIC_FILE"][0]),
                        os.path.join(ROOT_DIR, self.dic_path["PATH_TO_MODEL"], "round_0.h5"))
        elif self.dic_exp_conf["AGGREGATE"]:
            self.agent.q_network.save(ROOT_DIR + '/' + "model/initial", "aggregate.h5")
